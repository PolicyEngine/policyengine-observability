...
def _render(content: str, values: dict[str, str]) -> str:
    ...
    if unresolved:
        raise SystemExit(
            "Unresolved deployment variables: " + ", ".join(unresolved)
        )
    return rendered
...
def render_deployment(output_directory: Path) -> None:
    ...
    for source_name, output_name in TEMPLATES:
        ...
        rendered_files.append(
            (output_name, _render(source.read_text(), values))
        )
    ...
    print(
        f"Rendered {len(TEMPLATES)} deployment files using: {variable_names}"
    )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Destination for rendered files (default: deploy/gcp/rendered)",
    )
